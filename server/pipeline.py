...
error to the customer and logs the traceback server-side. jobs/<id>/ is always
removed in finally so the disk doesn't fill.
...
def process_job(job_id, email, original_name):
    ...
    try:
        ...
        transcribe(input_mp3, transcript_path, config.WHISPER_MODEL)
